...
//                   const char *max_label, double *max_value, double
//                   lower_bound, double upper_bound, const char *format =
//                   "%.2f") {
...
//   filter.known_map.assign(sim.true_landmarks.begin(),
...
//   draw->AddTriangle(plot_to_pixels(tip), plot_to_pixels(p1),
//   plot_to_pixels(p2),
//                     outline, 1.5f);
// }
//
...
//       0.5 * std::max(bounds.max_x - bounds.min_x, bounds.max_y -
//       bounds.min_y);
...
//                           bounds.max_y + pad);
//   else
//     mcl_.init_uniform(particle_count_, bounds.min_x - pad, bounds.max_x +
//     pad,
...
//                          omp_thread_limit_)) {
//       omp_thread_count_ = std::clamp(omp_thread_count_, 1,
//       omp_thread_limit_); omp_set_num_threads(omp_thread_count_);
//     }
...
//                             ? compute_world_bounds(sim_,
//                             estimate_trajectory_,
//                                                    mcl_omp_)
//                             : compute_world_bounds(sim_,
//                             estimate_trajectory_,
//                                                    mcl_);
...
//                   rgba(13, 38, 76, 255));
//   draw->AddCircleFilled(plot_to_pixels(Eigen::Vector2d(mean.x,
//   mean.y)), 4.0f,
...
//     ImPlot::PlotLine("error", history_step_.data(),
//     pose_error_history_.data(),
...
//       const double update_mean = mean(perf.measure_ms) +
//       mean(perf.observe_ms); ImGui::TableNextRow();
