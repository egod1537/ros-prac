...
SimView::SimView(GLFWwindow *win, Sim2D &sim, MCL &mcl, SimConfig &config)
  ...
  sync_mcl_known_map_();
  if (!mcl_.particles.empty())
    particle_count_ = static_cast<int>(mcl_.particles.size());
  else
...
void SimView::init_mcl_uniform_() {
  ...
  constexpr double pad = 1.0;
  std::uniform_real_distribution<double> ux(bounds.min_x - pad,
                                            bounds.max_x + pad);
  std::uniform_real_distribution<double> uy(bounds.min_y - pad,
                                            bounds.max_y + pad);
  std::uniform_real_distribution<double> uth(-kPi, kPi);
  const double inv_m = 1.0 / static_cast<double>(particle_count_);

  mcl_.particles.clear();
  mcl_.particles.reserve(static_cast<std::size_t>(particle_count_));
  for (int i = 0; i < particle_count_; ++i) {
    Particle p;
    p.x = ux(mcl_.rng);
    p.y = uy(mcl_.rng);
    p.theta = uth(mcl_.rng);
    p.weight = inv_m;
    p.log_weight = 0.0;
    mcl_.particles.push_back(p);
  }
...
void SimView::sync_mcl_known_map_() {
  mcl_.known_maps.assign(sim_.true_landmarks.begin(), sim_.true_landmarks.end());
}

void SimView::observe_mcl_(const std::vector<Observation> &obs) {
  const int m = static_cast<int>(mcl_.particles.size());
  if (m <= 0)
    return;

  const double inv_r2 = 1.0 / (mcl_.sigma_r * mcl_.sigma_r);
  const double inv_phi2 = 1.0 / (mcl_.sigma_phi * mcl_.sigma_phi);

  double max_log = -std::numeric_limits<double>::infinity();
  for (Particle &p : mcl_.particles) {
    double log_w = 0.0;

    for (const Observation &z : obs) {
      if (z.id < 0 || static_cast<std::size_t>(z.id) >= mcl_.known_maps.size())
        continue;

      const Eigen::Vector2d &lm = mcl_.known_maps[static_cast<std::size_t>(z.id)];
      const double dx = lm.x() - p.x;
      const double dy = lm.y() - p.y;
      const double r_hat = std::hypot(dx, dy);
      const double phi_hat = wrap(std::atan2(dy, dx) - p.theta);
      const double dr = z.r - r_hat;
      const double dphi = wrap(z.phi - phi_hat);
  ...
  if (sum_exp > 0.0 && std::isfinite(sum_exp)) {
    ...
    const double log_uniform = -std::log(static_cast<double>(m));
    for (Particle &p : mcl_.particles) {
      p.weight = uniform;
  ...
  if (mcl_.effective_n() < 0.5 * static_cast<double>(m)) {
    mcl_.resample();
    ++resample_count_;
  }
}
...
void SimView::render_config_() {
  ...
  ImGui::Text("M: %d", static_cast<int>(mcl_.particles.size()));
  ImGui::Text("Effective N: %.1f", mcl_.effective_n());
  ImGui::Text("Resamples: %d", resample_count_);
...
void SimView::render_control_() {
  ...
  ImGui::Text("Known map: %d", static_cast<int>(mcl_.known_maps.size()));
}
...
void SimView::render_effective_n_() {
  ...
  const double ymax =
      std::max(1.0, static_cast<double>(mcl_.particles.size()));
  ...
  if (!effective_n_history_.empty()) {
    ...
    const double threshold =
        0.5 * static_cast<double>(mcl_.particles.size());
